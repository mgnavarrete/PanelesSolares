...
#def get_thermal_dji(n):
def get_thermal_dji(path):
    input_file = f'test_2.raw'
    cmd = ['./dji_thermal_sdk_v1.4_20220929/utility/bin/windows/release_x86/dji_irp.exe', '-s', path, '-a', 'measure', '-o', f'{input_file}', '--measurefmt', 'float32']
    subprocess.call(cmd)
    npimg = np.fromfile(input_file, dtype=np.float32)
    imageSize = (512, 640)
    npimg = npimg.reshape(imageSize)
    return npimg
    #return get_thermal_djip

def get_thermal_flir(path):
    [temperaturas, metadata] = get_temp_from_flir(path)
    #npimg = []
    return temperaturas
