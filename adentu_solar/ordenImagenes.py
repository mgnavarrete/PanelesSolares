    ...
    for Pic in temp_archivos:
        if (verificador_extension(Pic)):
            ...
            if (fechaHora_anterior is not None):
                ...
                timedelta_temp = timedelta(seconds=30)
...
def ordenar_imagenes():

    seleccion = input('''Seleccione el orden en que están las imágenes:
        
        1 - CAMPO/VUELO - ZONA/VUELO
        2 - CAMPO/MISION_VUELO - ZONA_MISION/VUELO                 
    ...
    if Orden == "CAMPO_VUELO":
        niveles = 2
        list_niveles = ["", "v"]
        dir0 = filedialog.askdirectory(title='Seleccione directorio con imágenes de un CAMPO, separadas por vuelo (orden: PLANTA/CAMPO/VUELO/*.jpg)')
    elif Orden == "ZONA_MISION_VUELO":
        niveles = 3
        list_niveles = ["", "", "v"]
        dir0 = filedialog.askdirectory(title='Seleccione directorio con imágenes de una ZONA, separadas por misión y vuelo (orden: PLANTA/ZONA/MISION/VUELO/*.jpg)')
    ...
    for dir1 in os.listdir(dir0):
        ...
        if Orden == "CAMPO_VUELO":
            ...
            str_CT = dir0.split('/')[-1]  # 'NCU20'
            str_M = dir1  # "105MEDIA"
            for dir2 in os.listdir(dir0 + '/' + dir1):
                str_V = dir2  # "105MEDIA"
                im_path = dir0 + '/' + dir1 + '/' + dir2 # D:/ProcesamientoEnel_2023-10/Imagenes/Plantas/VDS/Z09/M2/DJI_202312111429_018_UgCS-Misiónde05-12-20231726KML62
                # PlantaDir = join(dir0.split('/'))[:-1]
                # CTdir = dir0
                # Vdir = dir0 + '/' + dir1
                # for Pic in os.listdir(Vdir):
