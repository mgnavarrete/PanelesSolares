...
def real_detect_new(path_save):
    ...
    for n in range(len(polygons)):
        ...
        name0.append(str(n))
        name1.append('_'.join(polygons[n]['Names'].split('_')[:-1]))
    ...
    aux_ = [x for x in polygons if 'Tipo X ' in x['falla']]
    for n in aux_:
        polygons.remove(n)

    aux_.sort(reverse=False, key=lambda x: (x['Names']))
    ...
    if len(aux_):
        ...
        for pol in aux_:
            if comparacion == pol['Names']:
                ...
                shutil.copy(path_images+f'/{comparacion.split("_")[2]}/{comparacion}.JPG',path_save+f'/detect/{temporal[0]["data"]["name"]}')
