...
def detect_fast(im_path, path_labels, path_obj, path_save, names):
    ...
    #bbox_path = [path_labels + "/v" + l.split("/")[-2] + "_" + l.split("/")[-1].replace('.JPG', '.txt') for l in im_path]
    bbox_path = [path_labels + "/v" + l.split("/")[-2][0] + "_" + l.split("/")[-1].replace('.JPG', '.txt') for l in im_path]
    ...
    for imp, bp in zip(im_path, bbox_path):
        ...
        if bboxes.__len__() > 0:
            ...
            for _b in bboxes:
                ...
                _name = name.replace(extension, f'_{falla_id}{extension}')
                to_file_bbox += f'"name" : "{_name}", "dd": {dd}, "falla_id": {falla_id}' + '}'
                falla_id += 1
                to_file_bbox = to_file_bbox.replace('}{', '},{')
            ...
            with open(path_save + f'/detect/{imp.split("/")[-1].replace(extension, ".json")}', 'w') as f:
                f.write('[')
                f.write(to_file_bbox)
                f.write(']')
            # imi = to_uint8g(im)
